...
namespace Calcpad.Wpf
{
    internal class HighLighter
    {
        ...
        // No es readonly: el tema Oscuro lo cambia (ver ApplyTheme).
        private static SolidColorBrush BackgroundBrush = new(Color.FromArgb(160, 240, 248, 255));
        ...
        /// Misma paleta que Hekatan Lab, para que los dos programas se lean igual.
        /// Tras llamarlo hay que RE-RESALTAR el editor: los Run existentes conservan el brush
        /// del tema anterior (en Oro, Const y Function son NEGROS → invisibles en Oscuro).</summary>
        internal static void ApplyTheme(bool dark)
        {
            IsDark = dark;
            if (dark)
            {
                Colors[(int)Types.None]     = Frz(0x8a, 0x83, 0x74);   // gris cálido apagado
                Colors[(int)Types.Const]    = Frz(0xe8, 0xe2, 0xd4);   // números → hueso
                Colors[(int)Types.Units]    = Frz(0x4e, 0xc9, 0xb0);   // turquesa
